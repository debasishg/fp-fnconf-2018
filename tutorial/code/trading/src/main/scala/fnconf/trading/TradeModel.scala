...
trait TradeModel {this: RefModel =>
  ...
  val taxFeeForTrade: Trade => Option[List[TaxFeeId]] = {trade =>
  ...
  def validateAccount(account: Account): Either[String, Account] = Right(account)
  def validateInstrument(instrument: Instrument): Either[String, Instrument] = Right(instrument)

  def validateUnitPrice(price: BigDecimal): Either[String, BigDecimal] = 
    if (price < 0) Left(s"Unit price $price has to be positive")
    else Right(price)
  ...
  def makeTrade(account: Account, 
    ...
    quantity: BigDecimal,
    td: Date = today, 
    vd: Option[Date] = None): Either[String, Trade] = for {

    _ <- validateAccount(account)
    _ <- validateInstrument(instrument)
    _ <- validateUnitPrice(unitPrice)

  } yield Trade(account, instrument, refNo, market, unitPrice, quantity, td, vd)
}
